        ...
        public bool Registrar(Prestamo objeto)
        {
            ...
            using (SqlConnection oConexion = new SqlConnection(Conexion.CN))
            {
                try
                {
                    ...
                    cmd.Parameters.AddWithValue("IdSolicitud", objeto.oLibro.IdSolicitud);
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        while (dr.Read())
                        {
                            Lista.Add(new Prestamo()
                            {
                                ...
                                oLibro = new Libro() { oTsolicitud = new Tsolicitud(){IdTiposolicitud = Convert.ToInt32(dr["IdTiposolicitud"])}},
